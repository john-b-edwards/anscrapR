...
scrape_cfb_game_odds <-
  function(week, season_type, season, division = "FBS") {
    stopifnot(season >= 2017)

    odds_json <- jsonlite::fromJSON(
      glue::glue(
        "https://api.actionnetwork.com/web/v1/scoreboard/ncaaf?division={division}&week={week}&season={season}&seasonType={season_type}"
      )
    )
