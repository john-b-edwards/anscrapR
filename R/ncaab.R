#' @title scrape_ncaab_game_odds
#'
#' @description Obtains historical and future NCAA MBB odds from the Action Network
#'
...
scrape_ncaab_game_odds <- function(date = Sys.Date()) {
  odds_json <- jsonlite::fromJSON(
    glue::glue(
      "https://api.actionnetwork.com/web/v1/scoreboard/ncaab?division=D1&tournament=0&date={gsub('-','',date)}"
    )
  )
